...
export default class MessagesList extends Component {
    ...
    doArchiveMessage = (index) => {
        const archivedMessages = [...this.state.archivedMessage];
        archivedMessages[index] = !this.state.archivedMessage[index];

        this.setState({
            archivedMessage: archivedMessages
        })
    };

    render() {
        const { messages, showMessage, archivedMessage} = this.state;
        return (
            <ul className="messages__list">
                {messages.length >= 1 && messages.map((message, index) =>
                    <li className="message " key={index}>
                        {!archivedMessage[index] &&
                        <div>
